...
class CocoonTestsPortfolios(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            [
                "ApiException due to invalid scope",
                "p rime_broker_test", # not the space between p and r, this should cause a server side response error
                "data/metamorph_portfolios-unique.csv",
                {
                    "code": "FundCode",
                    "display_name": "display_name",
    ...
    def test_load_from_data_frame_a_portfolios_failure(
        ...
        self.assertEqual( len(responses[0][file_type]['errors']), 3)
        
        self.assertTrue( isinstance(responses[0][file_type]['errors'][0], lusid.exceptions.ApiException ))
        self.assertTrue( isinstance(responses[0][file_type]['errors'][1], lusid.exceptions.ApiException ))
        self.assertTrue( isinstance(responses[0][file_type]['errors'][2], lusid.exceptions.ApiException ))
        
        # body is a str, not a type
        # avoided a hard coded message check as this likely to break wen error message is changed
        self.assertTrue( "scope" in responses[0][file_type]['errors'][0].body)
        self.assertTrue( "scope" in responses[0][file_type]['errors'][1].body)
        self.assertTrue( "scope" in responses[0][file_type]['errors'][2].body)
