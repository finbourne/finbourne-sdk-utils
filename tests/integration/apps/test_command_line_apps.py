...
class AppTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.factory = lusid.SyncApiClientFactory( )
            ...
            for portfolio in cls.groups_test_list:
                cls.transaction_portfolios_api.upsert_transactions(
                    ...
                    flush_test_data.gen_transaction_data(
                        2, datetime.datetime(2020, 2, 14, 0, 0, tzinfo=tzutc()).isoformat().replace("+00:00", "Z")
                    ),
                )
    ...
    def test_upsert_portfolios_withvalid_mapping(self):
        ...
        factory = lusid.SyncApiClientFactory( )
    ...
    def test_flush_between_dates(self, _, args, expected_txn_count):
        ...
        dates = [
            datetime.datetime(2020, 2, 12, 0, 0, tzinfo=tzutc()).isoformat().replace("+00:00", "Z"),
            datetime.datetime(2020, 2, 14, 0, 0, tzinfo=tzutc()).isoformat().replace("+00:00", "Z"),
            datetime.datetime(2020, 2, 19, 15, 0, tzinfo=tzutc()).isoformat().replace("+00:00", "Z"),
        ]
    ...
    def test_flush_with_failed_responses(self, _, test_fail, mock_lusid_cancel_txns):
        ...
        transactions = flush_test_data.gen_transaction_data(
            150, datetime.datetime(2020, 2, 14, 0, 0, tzinfo=tzutc()).isoformat().replace("+00:00", "Z")
        )
        ...
        batch_count = len(
            flush_transactions.transaction_batcher_by_character_count(
                args.scope,
                args.portfolio,
                self.factory._SyncApiClientFactory__api_client.configuration.host,
