...
class TestCocoonDateOrCutLabel:
    @classmethod
    def setup_class(cls) -> None:
        cls.logger = logger.LusidLogger(os.getenv("FBN_LOG_LEVEL", "info"))

    @pytest.mark.parametrize(
        "test_name, datetime_value, expected_outcome",
        [
            (
    ...
    def test_dateorcutlabel(self, test_name, datetime_value, expected_outcome):
        ...
        assert expected_outcome == str(date_or_cut_label.data)

    @pytest.mark.parametrize(
        "test_name, datetime_value, custom_format, expected_outcome",
        [
            (
                "YYYY-mm-dd_dashes",
                "2019-09-01",
